...
class CNNBaseModel(object):
    ...
    def conv2d(inputdata, out_channel, kernel_size, padding='SAME',
        ...
        with tf.variable_scope(name):
            ...
            if split == 1:
                ...
                inputs = tf.split(inputdata, split, channel_axis)#按维切割
    ...
    @staticmethod
    def depthwise_conv2d_p(inputdata, out_channel, kernel_size, padding='SAME',
               stride=1, w_init=None, b_init=None,
               split=1, use_bias=True, data_format='NHWC', name=None):
        with tf.variable_scope(name):
            in_shape = inputdata.get_shape().as_list()
            ...
            """ kernel_size是list? """
            if isinstance(kernel_size, list):
                filter_shape = [kernel_size[0], kernel_size[1]] + [in_channel / split, 1]
            else:
                filter_shape = [kernel_size, kernel_size] + [in_channel / split, 1]
            ...
            if split == 1:
                conv = tf.nn.depthwise_conv2d(inputdata, w, strides, padding, data_format=data_format)
            #    print(conv.shape)
            if use_bias:
                ret = tf.nn.bias_add(conv, b, data_format=data_format)
            else:
                ret = conv
