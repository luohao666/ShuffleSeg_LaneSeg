...
from lanenet_model import shufflenet_merge_model
...
SHUFFLE_MEAN = [73.29132098, 83.04442645, 72.5238962]
...
def test_lanenet(image_path, weights_path, use_gpu=1):
    """
    ...
    image = image - SHUFFLE_MEAN
    log.info('图像读取完毕, 耗时: {:.5f}s'.format(time.time() - t_start))

    input_tensor = tf.placeholder(dtype=tf.float32, shape=[1, 256, 512, 3], name='input_tensor')
    phase_tensor = tf.constant('test', tf.string)

    net = shufflenet_merge_model.Shuffle_LaneNet(net_flag='shuffle', phase=phase_tensor)
...
def test_lanenet_batch(image_dir, weights_path, batch_size, use_gpu=1, save_dir=None):
    """
    ...
    image_path_list = glob.glob('{:s}/**/*.jpg'.format(image_dir), recursive=True) + \
					  ...
					  glob.glob('{:s}/**/*.bmp'.format(image_dir), recursive=True)
    ...
    with sess.as_default():
        ...
        for epoch in range(epoch_nums):
            ...
            image_list_epoch = [tmp - SHUFFLE_MEAN for tmp in image_list_epoch]
            t_cost = time.time() - t_start
            ...
            cluster_time = []
            mask_image = np.zeros(shape=[binary_seg_images[0].shape[0], binary_seg_images[0].shape[1], 3], dtype=np.uint8)
            point_size = 1
            ...
            for index, binary_seg_image in enumerate(binary_seg_images):
                ...
                idx = np.where(binary_seg_image == 255)
                for i in range(len(idx[0])):
                    cv2.circle(mask_image,(idx[1][i],idx[0][i]),point_size,point_color,thickness)
                #简单的数据处理
