...
class Shuffle_LaneNet(cnn_basenet.CNNBaseModel):
    """
    实现语义分割模型
    """
    def __init__(self, phase, net_flag='shuffle'):
        ...
        """
        super(Shuffle_LaneNet, self).__init__()
        ...
        with tf.variable_scope(name):
            ...
            disc_loss, l_var, l_dist, l_reg = \
                lanenet_discriminative_loss.discriminative_loss(
                    pix_embedding, instance_label, 4, image_shape, 0.45, 4.0, 1.0, 1.0, 0.001)#delta_a=0.5,delta_d=3.0
...
if __name__ == '__main__':
    model = Shuffle_LaneNet(tf.constant('train', dtype=tf.string))
